...
st.title("Modelo CAPM para Evaluación Financiera | Por Richard Jammer")
st.markdown("Herramienta de análisis de Beta y Costo de Capital para empresas.")
...
# Inputs
st.markdown("### 📈 Tasa libre de riesgo (Rf) - FRED 10Y")
rf = st.number_input("", value=rf, step=0.0001, format="%.4f", disabled=True)

st.markdown("### 💹 Total Equity Risk Premium (ERP) - Perú")
erp = st.number_input("", value=erp_peru, step=0.0001, format="%.4f", disabled=True)

st.markdown("### ⚖️ Beta (β) de Metals & Mining (Damodaran)")
beta = st.number_input("", value=beta_desapalancado, step=0.01, disabled=True)



# Usar los valores directamente en el cálculo
ke = rf + beta_desapalancado * erp_peru
...
fig = px.line(
    ...
    title="Relación entre Beta y Costo de Capital - Mayor riesgo (beta) mayor Ke (rentabilidad exigida)"  # Título del gráfico
)
